...
## Helper: prep summary_df slice for plotting.
## Converts separation to an ordered factor (so x-axis is ordered) and
## event_rate_target to a labelled character (so facet_grid treats it as
## discrete and the labeller has no numeric/factor ambiguity).
...
## Helper: same for raw results_df slices
...
fig_ksel <- prep_summary(summary_df) %>%
  ...
  labs(title = "K-Selection Accuracy",
       subtitle = "Proportion of replicates where CV correctly identified K_true  |  dashed = chance",
       x = "Cluster Separation", y = "P(K\u0302 = K_true)") +
  theme_gemcox()

ggsave("fig1_K_selection.pdf", fig_ksel, width = 14, height = 6)

## ---- Figure 2: ARI (cluster recovery) ----
fig_ari <- prep_results(results_df) %>%
  filter(!is.na(ARI), K_true > 1, K_true == K_selected) %>%
  droplevels() %>%
  ...
  labs(title = "Cluster Recovery (Adjusted Rand Index)",
       subtitle = "n = 117; conditioned on correct K selection",
       x = "Cluster Separation", y = "ARI") +
  theme_gemcox()

ggsave("fig2_ARI.pdf", fig_ari, width = 9, height = 5)

## ---- Figure 3: C-index gain over single Cox ----
fig_gain <- prep_summary(summary_df, n_val = 117) %>%
  ggplot(aes(x = separation, y = cindex_gain_mean,
             ymin = cindex_gain_mean - cindex_gain_sd,
             ymax = cindex_gain_mean + cindex_gain_sd,
             color = separation, group = separation)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
  geom_pointrange(linewidth = 0.6) +
  facet_grid(event_rate_label ~ K_label) +
  scale_color_manual(values = sep_colors, name = "Separation") +
  labs(title = "C-Index Gain over Single Cox (K = 1)",
       subtitle = "Mean \u00b1 SD across 200 replicates; n = 117",
       x = "Separation", y = "\u0394C-index (GeM-Cox minus Cox)") +
  theme_gemcox()

ggsave("fig3_cindex_gain.pdf", fig_gain, width = 9, height = 5)

## ---- Figure 4: Effect of n and feature correlation ----
fig_n_rho <- summary_df %>%
  filter(K_true == 2, separation == "med") %>%
  mutate(
    rho_label        = paste0("\u03c1 = ", feat_corr),
    n_label          = paste0("n = ", n),
    event_rate_label = paste0("Event rate: ", event_rate_target)
  ) %>%
  pivot_longer(cols = c(K_sel_accuracy, ARI_mean, cindex_gain_mean),
               names_to = "metric", values_to = "value") %>%
  mutate(metric = recode(metric,
                         K_sel_accuracy   = "K Selection Accuracy",
                         ARI_mean         = "ARI (cluster recovery)",
                         cindex_gain_mean = "\u0394C-index"
  )) %>%
  ggplot(aes(x = n_label, y = value,
             color = rho_label, group = rho_label)) +
  geom_line() + geom_point(size = 3) +
  facet_grid(metric ~ event_rate_label, scales = "free_y") +
  scale_color_manual(values = c("\u03c1 = 0.2" = "#3a7a3a",
                                "\u03c1 = 0.5" = "#c04060"),
                     name = "Feature corr.") +
  labs(title = "Effect of Sample Size and Feature Correlation",
       subtitle = "K_true = 2, separation = medium",
       x = "Sample size", y = "Value") +
  theme_gemcox()

ggsave("fig4_n_rho.pdf", fig_n_rho, width = 9, height = 7)

## ---- Figure 5: Gamma selection distribution ----
fig_gamma <- prep_results(results_df) %>%
  filter(!is.na(gamma_selected)) %>%
  droplevels() %>%
  ggplot(aes(x = factor(gamma_selected), fill = separation)) +
  geom_bar(position = "dodge") +
  facet_grid(K_label ~ event_rate_label) +
  scale_fill_manual(values = sep_colors, name = "Separation") +
  labs(title = "Selected \u03b3 Distribution",
       subtitle = "How often each \u03b3 value is chosen by CV; n = 117",
       x = "\u03b3 (survival weight)", y = "Count") +
  theme_gemcox()

ggsave("fig5_gamma_selected.pdf", fig_gamma, width = 10, height = 6)

message("\n========== SIMULATION COMPLETE ==========")
message("Output files:")
message("  GeMCox_simulation_raw.rds")
message("  GeMCox_simulation_summary.rds / .csv")
message("  fig1_K_selection.pdf")
message("  fig2_ARI.pdf")
message("  fig3_cindex_gain.pdf")
message("  fig4_n_rho.pdf")
message("  fig5_gamma_selected.pdf")
